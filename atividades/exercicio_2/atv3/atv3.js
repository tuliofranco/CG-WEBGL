...
function initializeWebGL() {
  ...
  // Variável para a espessura do traçado
  let lineWidth = 5.0; // Espessura inicial
  ...
  function main() {
    ...
    let functionMode = 'draw'; // 'draw', 'color', 'thickness'
    ...
    gl.uniform1f(gl.getUniformLocation(program, 'pointSize'), lineWidth);
    ...
    // Evento de teclado para mudar a cor, espessura e o modo de desenho
    document.addEventListener('keydown', (event) => {
        const key = event.key;
  
        if (functionMode === 'color') {
            if (key >= '0' && key <= '9') {
            ...
            // Retorna ao modo de desenho e reseta variáveis
            functionMode = 'draw';
            isStartSet = false;
            trianglePoints = [];
        } else if (functionMode === 'thickness') {
            if (key >= '1' && key <= '9') {
                lineWidth = parseInt(key) * 2; // Multiplica por 2 para tornar a espessura mais visível
                // Redesenha a figura atual com a nova espessura
                if (lastVertices.length > 0) {
                    setupLineBuffer(gl, program, lastVertices);
                    prepareCanvas(gl);
                    gl.uniform1f(gl.getUniformLocation(program, 'pointSize'), lineWidth);
            ...
            if (key === 'r' || key === 'R') {
                ...
                functionMode = 'color';
                // Reseta variáveis ao entrar no modo de mudança de cor
                isStartSet = false;
                trianglePoints = [];
            } else if (key === 'e' || key === 'E') {
                functionMode = 'thickness';
                // Reseta variáveis ao entrar no modo de mudança de espessura
                isStartSet = false;
                trianglePoints = [];
            }
        }
